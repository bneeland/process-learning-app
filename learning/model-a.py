...
LabelledDataset_Original = pd.read_csv("../mining/output_sequence/outputsequence-a.csv")
...
try:
    w = csv.writer(open("saved_data/LabelIDs-a.csv", "w"))
...
try:
    w = csv.writer(open("saved_data/EventIDs-a.csv", "w"))
...
try:
    w = csv.writer(open("saved_data/x_height-x_width-a.csv", "w"))
...
# Save the model
try:
    Model.save("saved_models/PLA_Learning_CNN-Model-a.h5")
    print("Model saved.")
except:
